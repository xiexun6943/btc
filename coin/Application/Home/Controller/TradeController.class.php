...
class TradeController extends HomeController
{
	...
	{
		parent::_initialize();
		$allow_action=array("index","bborder","bbhistoryorder","tradelist","ordinary","trans","transinfo","upbbbuy","getprice","upbbsell","clearorder","tradebill","billinfo","specialty");
	...
	public function clearorder($oid=null){
	    ...
	    $upre = M("bborder")->where(array('id'=>$oid))->save(array('status'=>3,'tradetime'=>date("Y-m-d H:i:s",time())));
	    ...
	    if($upre && $decre && $incre){
	        $this->ajaxReturn(['code'=>1,'info'=>L('订单已撤消')]);
	    }else{
	        $this->ajaxReturn(['code'=>0,'info'=>L('订单撤消失败')]);
	    }
	...
	//全部委托订单及交易记录
	public function bbhistoryorder(){
	     $uid = userid();
	    if($uid <= 0){
	        $this->redirect('Login/index');
	    }
...
//                $close = $pdata['close'];//现价
	...
	public function index(){
        ...
        if($symbol != ''){
            ...
            if(!empty($list)){
                $symbol=mb_substr($list['name'],0,-5);
            }else{
                $symbol = "BTC";
            }
        }else{
	    ...
	    $this->assign('coinname',$symbol.'/USDT');
        $this->assign('symbol',$symbol);
        $this->assign('symbolup',strtoupper($symbol));
        ...
        //全部交易记录
        if ($uid) {
            $bblist = M("bborder")->where("uid = {$uid} and status != 1")->limit(5)->order('id desc')->select();
            $bbcount = M("bborder")->where("uid = {$uid} and status != 1")->count();
            //全总未交易委托
            $bblist1 = M("bborder")->where(array('uid'=>$uid,'ordertype'=>1,'status'=>1))->limit(5)->order('id desc')->select();
            $bbcount1 = M("bborder")->where("uid = {$uid} and status != 1")->count();
        }else{
            $bblist = [];
            $bblist1 = [];
            $bbcount = 0;
        ...
        $this->assign('bbcount',$bbcount);
        $this->assign('bblist1',$bblist1);
        $this->assign('bbcount1',$bbcount1);
