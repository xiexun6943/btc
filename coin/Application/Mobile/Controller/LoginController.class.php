...
class LoginController extends MobileController
{
	...
	{
		parent::_initialize();	$allow_action=array("index","register","upregister","loginsubmit","loginout","findpwd","findpaypwd",
        "webreg","loption","setlang","lhelp","sendcode","findsendcode","resetpwd",'registers',"sendSmsCode",'smsregister');
	...
	public function registers()
    {
        ...
        if ($type =='' || $account == '') {
            return $this->redirect("Login/registers");
        }
        if($qrcode != ''){
            $this->assign('qrcode',$qrcode);
        }
...
//	    $smstemple = trim($config['smstemple']);
        ...
        $mail->Body = '[BitoPro] '.L("验证码5分钟内有效,").L("您的验证码是:").$desc_content;;
		if(!$mail->send()){
			return 0;
		}else{
			return 1;
    ...
    public function sendSmsCode(){
        if($_POST){
            ...
            $code = rand(10000,99999);
            $desc_content='[BitoPro] '.L("验证码5分钟内有效,").L("您的验证码是:").$code;
