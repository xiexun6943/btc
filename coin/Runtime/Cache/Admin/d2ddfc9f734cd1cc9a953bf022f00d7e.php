...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			...
			<div class="fr">
				<button class="btn btn-warning" onClick="location.href='<?php echo U('Issue/index');?>'">初始化搜索</button>
			</div>
		</div>
		<div class="cf">
			<div class="fl">
				<a class="btn  btn-success" href="<?php echo U('Issue/edit');?>" style="    padding: 6px 30px;">新 增</a>
				<button class="btn ajax-post confirm btn-danger " url="<?php echo U('Issue/billdel',array('type'=>'1'));?>" target-form="ids">删 除</button>
			</div>
		</div>
			...
			<table class="">
				...
				<tbody>
					<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td><?php echo ($vo["buycoin"]); ?></td>
								<td><?php echo ($vo['num']*1); ?></td>
								<td><?php echo ($vo['ysnum']*1); ?></td>
								<td><?php echo ($vo['sellnum']*1); ?></td>
								<td><?php echo ($vo['price']*1); ?> <?php echo ($vo["buycoin"]); ?></td>
								<td><?php echo ($vo['allmax']*1); ?></td>
								<td><?php echo ($vo['lockday']); ?>天</td>
								<td>开启时间：<?php echo ($vo['starttime']); ?><br>结束时间：<?php echo ($vo['finishtime']); ?></td>
								<td><?php if(($vo["status"]) == "1"): ?>显示<?php else: ?>隐藏<?php endif; ?></td>
								<td><a href="<?php echo U('Issue/edit?id='.$vo['id']);?>" class="btn btn-primary btn-xs">编辑</a></td>
