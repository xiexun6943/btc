...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			...
			<?php if(!empty($name)): ?><span class="h2-title">>><a href="<?php echo U('User/adminUser');?>">管理列表</a></span><?php endif; ?>
		</div>
		<div class="cf">
			<div class="fl">
				<a class="btn btn-success " href="<?php echo U('User/adminEdit');?>">新 增</a>
				<button class="btn ajax-post btn-info " url="<?php echo U('User/adminStatus',array('type'=>'resume','mobile'=>'Admin'));?>" target-form="ids">启 用</button>
				<button class="btn ajax-post btn-warning " url="<?php echo U('User/adminStatus',array('type'=>'forbid','mobile'=>'Admin'));?>" target-form="ids">禁 用</button>
				<button class="btn ajax-post confirm btn-danger " confirm-msg="批量删除不可恢复,确定删除?" url="<?php echo U('User/adminStatus',array('type'=>'del','mobile'=>'Admin'));?>" target-form="ids">删 除</button>
			</div>
				...
				<div class="sleft">
					<form name="formSearch" id="formSearch" method="get" name="form1">
						...
						<select name="field" class="form-control" style="width:160px;float:left;margin-right:10px;">
							<option value="username" <?php if(empty($_GET['field'])): ?>selected<?php endif; ?>>用户名</option>
							<option value="moble" <?php if(($_GET['field']) == "moble"): ?>selected<?php endif; ?>>手机号码</option>
						</select>
		...
		<div class="data-table table-striped">
			<table class="">
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php if(($vo["role"]) == "1"): ?>管理员<?php endif; ?>
								<?php if(($vo["role"]) == "2"): ?>客服<?php endif; ?>
								</td>
							...
							<td>
								<a href="<?php echo U('User/adminEdit?id='.$vo['id']);?>" class="btn btn-primary btn-xs">编辑</a>
							</td>
...
<div style="display:none;">
	...
	<script type="text/javascript" charset="utf-8">
		//导航高亮
		highlight_subnav("<?php echo U('User/admin');?>");
	</script>