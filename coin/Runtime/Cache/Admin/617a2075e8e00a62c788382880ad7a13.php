...
<div id="main-content">
	...
	<div id="main" class="main">
		<div class="main-title-h">
			...
			<a class="btn btn-warning" onClick="location.href='<?php echo U('Trade/tyorder');?>'">初始化搜索</a>
		</div>
		<div class="cf">
				...
				<div class="sleft">
					<form name="formSearch" id="formSearch" method="get" name="form1">
						<select style="width: 120px; float: left; margin-right: 10px;" name="hyzd" class="form-control">
							<option value=""
							<?php if(empty($_GET['hyzd'])): ?>selected<?php endif; ?>
							>全部类型</option>
							<option value="1"
							<?php if(($_GET['hyzd']) == "1"): ?>selected<?php endif; ?>
							>买涨</option>
							<option value="2"
							<?php if(($_GET['hyzd']) == "2"): ?>selected<?php endif; ?>
							>买跌</option>
						</select>
			...
			<table class="">
				...
				<tbody>
                    <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>
						    <a href="<?php echo U('Trade/orderinfo_ty');?>?id=<?php echo ($vo["id"]); ?>">查看</a>
						</td>
...
<script type="text/javascript">
    function setwinloss(id){
        ...
        $.post("<?php echo U('Trade/setwinloss_ty');?>",
        {'id':id,'kongyk':kongyk},
        function(data){
            if(data.code == 1){
                alert(data.info);
...
<div style="display:none;">
	...
	<script type="text/javascript" charset="utf-8">
		//导航高亮
		highlight_subnav("<?php echo U('Trade/tyorder');?>");
	</script>