  ...
  <body>
      <script>
          $(function(){
              // 检测是否在WebView中打开
                function isWebView() {
                  // 检查是否存在WebView特有的用户代理标识
                  if (navigator.userAgent.match(/(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/i) ||
                      navigator.userAgent.match(/Android.*(wv|WebView)/i) ||
                      navigator.userAgent.match(/(iPhone|iPod|iPad|Android).*[Ww]eb[Vv]iew/i)) {
                    return true;
                  } else {
                    return false;
                  }
              ...
              const isIPhone = window.navigator.userAgent.includes("iPhone OS")
              let iphoneVer = ''
              let iphoneVerNumber = ''
              let isModernIPhone = false
              if(isIPhone === true){
                  iphoneVer = window.navigator.userAgent.indexOf("iPhone OS")
                  iphoneVerNumber = window.navigator.userAgent.substr(iphoneVer+9,5)
              }
              if(parseInt(iphoneVerNumber)>=15){
                  isModernIPhone = true
              }
              if(isModernIPhone===true&&isWebView()===true){
                  $('.blank_div').show()
                  $('.container-fluid').addClass('isModernIPhone')
              }else{
                  $('.blank_div').hide()
                  $('.container-fluid').removeClass('isModernIPhone')
              }
          })
      </script>
		...
		<div style="width:100%;height:90px;">
			...
			<a href="<?php echo U('Issue/index');?>">
				<div class="oplist">
					...
					<div class="optext">
						<span class="fzmm fcc" style="font-size: 12px;"><?php echo L('ieo');?></span>
					</div>
				</div>
			</a>
			...
			<a href="<?php echo U('Orepool/index');?>">
				<div class="oplist">
					...
					<div class="optext">
						<span class="fzmm fcc" style="font-size: 12px;">Staking</span>
					</div>
				</div>
			</a>

<!--			<a href="<?php echo ($clist["kefu"]); ?>">-->
				<a href="<?php echo U('Trade/kefu');?>">
				<div class="oplist">
					<div class="opimg" style="line-height:50px;">
	...
	<!--重点币行情区-->
	<div class="zixunlist" style="margin-top:10px;">
